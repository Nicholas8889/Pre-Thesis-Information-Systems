...
export async function CollectionsPage({
  ...
  return (
    <>
      <PageHeader
        title="Collections"
        description="Track planned and completed payment collection activities."
      />
            ...
            <form action={createCollectionTask} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <label className="text-sm font-medium text-slate-700 xl:col-span-3">
                ...
                <input
                  ...
                  defaultValue={
                    selectedInvoice
                      ? `Collect remaining payment of ${formatCurrency(
      ...
      <section className="rounded-md border border-line bg-white p-5 shadow-soft">
        ...
        {visibleCollectionTasks.length === 0 ? (
          <EmptyState
            message={
              activeTab === "done"
                ? "No completed collection tasks."
                : "No ongoing collection tasks."
            }
          />
        ) : (
...
export async function CustomerOutreachPage({
  searchParams
}: {
  searchParams?: Promise<SearchParams>;
}) {
    ...
    <>
      ...
      <section id="record-outreach" className="mb-6 rounded-md border border-line bg-white p-5 shadow-soft">
        ...
        {allCustomers.length === 0 ? (
          <EmptyState message="Add a customer before recording customer outreach." />
        ) : (
          <form
